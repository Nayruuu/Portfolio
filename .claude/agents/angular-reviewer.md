...
description: Angular 21 code review focused on the project conventions (standalone, zoneless, signals, native control flow, separate templates/styles, bilingual i18n, explicit accessibility, OnPush). Launch it after writing/editing Angular components or services.
...
- [ ] `effect()` with a timer/subscription → `onCleanup`; manual listeners/timers cleaned up via
      `DestroyRef`.
- [ ] i18n: text in both locale JSON files, `Content` type aligned — no hardcoded template text and
