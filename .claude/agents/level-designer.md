...
- **DOOM-map SCALE — sized by the floor's ROLE, never a handful of boxes.** Opener (M1, BUILT) = compact
  8-10 dense spaces (M1 is the density/dressing reference, NOT the size target — it has no exit at all: the M1 ⇄ M2 edge is the live passable seam);
...
- **Secrets (1-2 per floor).** Mechanism: an unmarked animated door (`doors[]`, `requiresCard: null`, wall
  ...
  (M3's own M9 exit sits behind its Secret 1 — the pattern to mirror if another hidden seam is ever added.)
...
- **Lighting = dread-via-CONTRAST, not brightness.** Fear comes from the interplay of dark and light —
  never uniform (a uniformly-lit office reads mundane; a uniformly-dim one reads flat). Author each
  sector's `light` (0-255) to a role and make ADJACENT sectors contrast (as-shipped values, after a global
  brightness tune): **DARK ~100** (corridors / stairwells / connectors / secrets — the black you hesitate to
  enter) · **DIM ~128** (main work floors — oppressive but navigable) · **SHADE ~113** (mazes, shadowed
  pockets) · **LIT ~190** (rooms with a reason to glow) · **POOL 210-255** (objectives + light SOURCES:
  windows/backdrops/NEON, focal islands — NEVER darkened). Scale the baseline by a **dread curve**: early
  floors keep a higher DIM (M1 lobby ~155, premium-but-with-shadow), deep floors go dark (M7/M8 ~110 with
  equipment-glow as the light in the dark, M9 darkest — bottoms ~88). "Bright = bait" is a legitimate lie
  (M9). Nothing pitch-black — floor ~88, enemies stay visible. (Recipe proven on M2 as the reference, then
  propagated across all 9 floors + a +18 global brightness lift, 2026-08-12.) For a MOVING dread cue, mark
  a sector via `Level.flicker` (a list of captured sector indexes) — a deterministic dying-fluorescent buzz
  on that sector's `light` (mostly on, brief sharp dips, tubes desync'd by seed); best on a lone dim/NEON
  fixture in the dark, one or two per floor (not everywhere). Shipped on M7's cable gallery + a hot aisle.
...
| M3 | RH / Human Resources (**BUILT** — `level-m3-hr.ts`, M2 ⇄ M3 graph airlock) | `CUBICLE`+`SCREEN` / `TECHNICAL` / `CARPET` | **manager (yellow) — introduced here** | HR floor — holds the **secret exit → M9**, the freight lift behind Secret 1 (wired) |
...
| M9 | Archives (SECRET) (**BUILT** — `level-m9-archives.ts`, M3 ⇄ M9 graph airlock behind the archives secret) | `DAMAGED` / `CEIL_DAMAGED` / `SLAB` | — | hidden derelict — the maze where LIGHT LIES (bright pools = ambushes, the dim path is true); pays the detour with the plasma PX-1 |
