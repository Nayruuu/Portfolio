...
- `use.baseURL: 'http://localhost:${port}'` where `port` = **`PW_PORT` env or 4200** — override it
  ...
  FR by default. EN is reached by clicking the `.nav__lang` "EN" toggle or via an `/en/...` deep link.
...
- `projects`: **three** — `chromium` (`devices['Desktop Chrome']`, runs every spec except the iOS one,
  via `testIgnore`), `mobile` (`devices['Pixel 5']`, 393×851) with `testMatch: /visual(-detail)?\.spec\.ts/`,
  so the mobile project **only re-runs the visual specs** to capture phone baselines, and `webkit`
  (`devices['iPhone 13']`) with `testMatch: /player-ios\.spec\.ts/` — the real **iOS Safari engine**, which
  the Chromium-based `mobile` project (device *emulation*, not WebKit) can't exercise. The behavioural specs
  drive nav controls that live in the **mobile drawer** at phone widths (closed by default), so they stay
  desktop-only. **Setup:** the `webkit` project needs `npx playwright install webkit` (the others use
  chromium).
...
### The 15 specs (behavioral + visual)
...
- `preferences.spec.ts` — theme toggle (`aria-label="Changer de thème"` → `<html data-theme="dark">`,
  default `light`); language switch (`.nav__lang` "EN" → "Home" visible, "Accueil" count 0).
- `seo.spec.ts` — `/fr/articles/etrangler-le-monolithe-dotnet`: title `/super-dev\.app/`, one
  `og:title`, `og:type="article"`, canonical ending the slug, one `hreflang="en"` alternate, and
...
- `overflow.spec.ts` — the mobile-first guard: at a **360px** viewport, asserts **no horizontal scroll**
  ...
  `/fr`, `/fr/{articles,series,about,stack,contact}`) **and on every article detail page in both
  languages** (slugs read from `content.fr.json` at runtime, so new articles are covered
...
Inputs: articles from `src/app/core/content/content.fr.json`; languages `['fr', 'en']`; output dir
`dist/super-dev-portfolio/browser`.
