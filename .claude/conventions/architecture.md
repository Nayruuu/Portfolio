...
| **shared** | `client/src/app/shared/` | Cross-feature components (`icon`, `code-block`, `inline-runs` — presentational; `like-bar` — the interactive vote bar, on both home and articles; `reviews` — the Malt-recos block, collapsed on home / expanded on `/about` via `[startExpanded]`; `command-palette` — the desktop-only ⌘K command palette; `content-tabs` — the route-backed Articles\|Séries toggle atop the two list pages). | `core`, `domain` |
...
- The sub-domain folders (`about/`, `article/`, `code/`, `contact/`, `discuss/`, `game/`, `i18n/`,
  `palette/`, `player/`, `project/`, `review/`, `series/`, `stack/`, `aria/`) are **internal organization only** —
...
- **One** barrel: `core/lib/index.ts`, `export *` from each `lib/` file (one line per file). A cohesive
  ...
  - `core/lib/game/` — **the whole self-contained embedded game engine** (§1 callout), grouped into **18**
    sub-folders by concern. The **pure logic** (100 %-tested): `enemy/` (the roster + the pure AI), `combat/`
    (hitscan / projectile / per-frame combat), `doors/`, `flicker/` (the deterministic dying-fluorescent light-flicker fn), `controls/`, `zone/` (the zone snapshot), `level/`
    ...
    **presentation** helpers in `presentation/` (the `DoomHud` / `WeaponView` / `ClimbView` imperative canvas
    ...
    `gpu-renderer`, `render-host`, `load-textures`), `input/` (the DOM-event `input-controller` + its pure `joystick`/`look` mappers), `boot/`
    ...
    (movement / look / touch / combat / enemy / pickup / door / timing constants). The barrel (`game/index.ts`)
...
was decomposed into this shape: `core/lib/game/` — the whole embedded engine — splits into **18** categorized
sub-folders (`boot/ combat/ controls/ doors/ enemy/ flicker/ input/ level/ levels/ painters/ presentation/
registry/ render/ sprites/ telemetry/ weapons/ world/ zone/`), while `features/bsp-demo/` is now just the thin **mount
...
domain/
  ...
  aria/     aria.ts
  article/  article.ts article-block.ts article-tag.ts articles-ui.ts indexed-article.ts inline-run.ts toc-entry.ts
...
core/
  ...
  services/   content/  game/  i18n/  palette/  player/  search/  seo/  share/  theme/  # one folder per service (+ its .spec)
...
features/home/
  ...
  lets-talk/  up-next/  video-meta/   # one folder per sub-component (like-bar and reviews are shared/)
...
`shared/icon/`, `shared/code-block/`, `shared/command-palette/`, `shared/content-tabs/`, `shared/inline-runs/`, `shared/like-bar/`, `shared/reviews/`; `layout/nav/`,
...
(presentational, plus the interactive `like-bar` vote widget, the `reviews` recos block, the desktop-only `command-palette`, and the route-backed `content-tabs` toggle) — there is **no `shared/game/`**. The game's presentational helpers (the `DoomHud` /
...
co-located with the component (e.g. `IconName`, the 30-name union, lives in the generated
