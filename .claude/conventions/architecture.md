...
|---|---|---|---|
| **domain** | `client/src/app/domain/` | Types, interfaces, value sets. The bilingual `Content` contract. **The innermost layer.** | **nothing** (no Angular, no RxJS, no other layer) |
...
| **layout** | `client/src/app/layout/` | The app shell: `nav`, `channel-header`, `tabs-bar`. | `core`, `domain`, `shared` |
...
- **One** barrel: `core/lib/index.ts`, `export *` from each `lib/` file (one line per file).
...
`Content` (in `content.ts`) is the FR/EN contract: the master interface both `content.fr.json` and
`content.en.json` must satisfy.
...
core/
  ...
  services/   content/  i18n/  theme/  player/  search/  seo/  viewport/  # one folder per service (+ its .spec)
  content/    content.fr.ts  content.en.ts  content.{fr,en}.json  article-bodies.ts
  lib/        index.ts + one pure function per file (+ constants.ts) — 100 % tested
```
...
- `core/content/` — **all text lives here as JSON**, exposed through a typed bridge (§5). Article
  **bodies** are real Markdown kept *out* of the JSON, under
  `client/src/content/articles/<slug>.{fr,en}.md`, imported as text by `article-bodies.ts`.
...
`shared/icon/`, `shared/code-block/`, `shared/inline-runs/`; `layout/nav/`,
...
`core/content/content.{fr,en}.ts` is the seam:
...
- `satisfies JsonContent` keeps the **compile-time completeness + FR/EN alignment** check (every
  field present, both languages structurally identical).
...
- `core/lib/site.ts` — the SEO/site constant set (`SITE_ORIGIN`, `SITE_NAME`, `DEFAULT_OG_IMAGE`,
  `AUTHOR`, `OG_LOCALE`, `TWIN_LANG`) grouped as one site-config unit.
...
co-located with the component (e.g. `IconName`, the 23-name union, lives in the generated
...
- **Language is a URL prefix via two explicit static trees** (`/fr`, `/en`) in `app.routes.ts`, each
  running a `langResolver` (`resolve: { lang }`) that syncs `I18nService` before render. It is
  **not** a `:lang` param — a parameter-first parent route breaks Angular's native prerenderer
  (empty `<router-outlet>`). `/` and unknown paths redirect to `/fr` with **static** strings. Both
  trees share the same **lazy** children (`langChildren()`).
- The **route is the source of truth for language**: `routerLink`s are prefixed with `i18n.lang()`;
  the FR/EN toggle **navigates** to the twin URL, it does not call `setLang` directly.
...
- **SEO logic lives in `core`**: `core/services/seo/seo.service.ts` sets title/OG/canonical/hreflang
  + `BlogPosting` JSON-LD, using pure helpers in `core/lib/site.ts` / `abs-url.ts` / `twin-path.ts` /
  `article-description.ts`. `AppComponent` sets the baseline; article-detail sets its own. The SEO
  service depends inward (on `core/lib` + `domain`) like any other `core` service.
