...
- Not linted: `dist`, `node_modules`, `.angular`, `coverage`, `e2e/__screenshots__`,
  `scripts/**`, `eslint-rules/**`, `wasm-proto/**`.
...
export const ARTICLE_TAGS = ['.NET', 'ANGULAR', 'AZURE', 'FLUTTER', 'DEVOPS'] as const;
...
```ts
export type CodeLang = 'csharp' | 'typescript' | 'yaml' | 'dart' | 'bash' | 'sql';
