...
FR+EN) — never selected with a language ternary. Both of these are banned:

```html
<!-- ✗ bypasses the typed bridge; breaks FR/EN alignment -->
...
```
```html
<!-- ✓ text comes from content.{fr,en}.json via the typed Content surface -->
{{ i18n.content().nav.home }}
```

A `lang()==='fr' ? … : …` ternary that picks **text** sidesteps `content.{fr,en}.json` and the
`Content` contract, so a string added in one language silently has no counterpart in the other —
exactly what the FR/EN-alignment check exists to prevent. Put the string in *both* JSON files and
read it from `i18n.content()`.
...
bridge mechanics (`satisfies JsonContent as Content`), the `Content` FR/EN contract, and the
...
> the `Content` FR/EN contract, and one-declaration-per-file / barrel rules are all in
