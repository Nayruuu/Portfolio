...
| `--code-mute` | `#83838a` | code (always dark) |
| `--code-comment` | `#8a8a90` | code (always dark) |
...
Nineteen partials. The `@use` name drops the leading `_` and `.scss` (`@use 'styles/tokens'` ⇒
`_tokens.scss`). Each is one cohesive concern; sizes below are the original byte counts (a sanity check
that you reproduced the whole file, not a fragment). Eighteen are in the cascade-significant `styles.scss`
...
| `_likebar.scss` | `.likebar-wrap`, `.likebar`, `.likebar button` (+ `:hover`/`.is-on`), `.likebar__divider`, `.likebar__count`. | — |
...
have NO `.scss` of their own — do not invent one for them.** There are **30** component `.scss` files
and **5** components with **no `styleUrl`** (they render only global-partial classes).
...
| `like-bar.component` | `.likebar-wrap`, `.likebar`, `.likebar__divider`, `.likebar__count` | `_likebar` |

Do **not** generate a `.scss` (or `styleUrl`) for any of these five. The `pulse`
...
|---|---|---|
| `layout/nav` | `.nav*` (brand, avatar, and `.nav__search` — now a `<button>` **trigger** that opens the ⌘K command palette, kept visually identical to the old search box: icon + placeholder text + `/` kbd) — hosts `<sd-prefs>` in `.nav__actions`. Hidden below `md` (`display:none`; the player owns the top of the phone screen), the sticky top bar returns at `bp.from(md)` | yes (7 rules) |
...
| `features/bsp-demo` | `.bsp-demo` (the standalone `/bsp` debug layout — flex column, centred, `gap`/`padding`); `&__viewport` (`position:relative`, `min(92vw,960px)`); `&__canvas` (`width:100%`, `image-rendering:pixelated` — the below-display framebuffer upscales crisp for the software-renderer look); `&__vignette` (radial-gradient edge-darkening overlay for oppressive/claustrophobic mood — `position:absolute; inset:0; pointer-events:none`, **no z-index** so DOM order alone paints it over the canvas yet under the HUD/buttons/readouts); `&__hud` (the composited **DOOM image HUD** `<canvas>`, `aria-hidden`; `inset:auto 0 0`, `height:22%`, `aspect-ratio:2117/404`, `margin-inline:auto`, `pointer-events:none` — the whole bar drawn in JS by `DoomHud`); grouped `&__exit, &__fullscreen` (top-right circular corner buttons — exit-door + native-fullscreen toggle, `right:10px`/`54px`; the fullscreen button only renders when the player passes `fullscreenAvailable`, never on `/bsp`); `&__fps` (top-left dev readout — FPS/ms/threads/texture source, mono green); `&__hint` (dev keyboard hint, mono); `&__controls` (top-left semi-transparent keyboard-controls recap, `display:none` by default) + `&__control` (`white-space:nowrap`). **Mobile twin-stick touch layer** (revealed under `@media (pointer: coarse)`, `display:none` on desktop; `&__canvas` carries `touch-action:none` so a thumb drag drives the game): the floating `&__joystick` + `&__thumb` (left-half move stick, drawn at the canvas-local touch origin), and the fixed `&__fire` (bottom-right, solid-disc `::after`) + `&__weapon` (`sd-icon` `layers`) buttons — all `aria-hidden`/`tabindex=-1`; on coarse the desktop `&__hint` is hidden. **Portrait cover** `&__rotate` (opaque `var(--bg)` full-cover under `@media (pointer: coarse) and (orientation: portrait)` that swallows touches) holding `&__rotate-icon` (`sd-icon` `gamepad`, `var(--accent)`) + `&__rotate-text` (i18n `gameRotate`) — a twin-stick FPS needs landscape. `:host-context(.player)` overrides make the game fill the player box (`:host` + `.bsp-demo` + `&__viewport`/`&__canvas` → `position:absolute; inset:0`, dropping the standalone centring/padding), **hide** the dev `&__fps`/`&__hint`, and **show** `&__controls`. Mounted `@if (game.running())` in place of `sd-player-stage`, and served standalone at `/bsp` | no |
...
| `features/articles/article-detail` | `.article-detail*` (incl. the `__reading` grid + sticky `__toc*` rail that join only at `bp.from(lg)`, the `__progress` reading bar pinned `top:0`, and the `__anchor` `#` heading-permalink), `.article-hero*`, `.series-ribbon*`, `.rel-card*` (NOT `.rel-card__sym` — hoisted) | no |
...
| `features/about/about` | `.about-grid` (2-col at `bp.from(md)`), `.about-bio*`, `.about-side*`, `.about-reviews` (dashed-rule top separator above the shared `sd-reviews` recos block) | no |
| `features/projects/projects` | `.proj-list` (1-col → 2-col at `bp.from(md)`), `.proj-card*` (the project-list card — distinct from the home `projects-scene` `.proj-card*`, isolated by emulated encapsulation) | no |
| `features/projects/project-detail` | `.proj-detail*` (back link, overline, name, lead, accent `metric` box, the optional case-study `__narrative` paragraphs, an optional `__image` `Aperçu` screenshot or code-block sample, `__highlights` list, stack chips, `Ressources` external-link row) | no |
...
| `shared/command-palette` | the desktop ⌘K palette: `.cmdk` fixed scrim (raw `rgb(0 0 0 / 55%)` + blur) → `&__panel` dialog; `&__search`/`__input` (16px, descriptive `gameRotate`-style placeholder via `content.palette.placeholder`, `::placeholder` in `--text-faint`) + `&__hint` (plain `esc`); `&__results` `role=listbox` with `&__group`/`__group-label` and the option `&__row`/`__row--active` (`--accent-glow` bg + inset `--accent` left bar); `&__row-icon` (mono line icon, **tinted per item** via `[style.color]` — articles/series carry their brand colour, pages/projects/actions a curated hue) + `&__row-text`/`__row-label`/`__row-sub` + `&__hit` (matched chars in `--accent`); `&__empty`; `&__footer`/`__foot`/`__key` (key hints). `@if (palette.open())` renders nothing closed (SSR-safe); no mobile trigger (desktop-only) | no |
| `shared/reviews` | `.reviews*` collapsed toggle row (22px overlapping initial discs, teaser, `via Malt` chip, chevron) + expanded `.review*` cards (40px disc on data-driven color, role pill, `pre-line` body); `[startExpanded]` seeds the state — collapsed on `home`, expanded on `/about` | — |
| `shared/content-tabs` | `.ctabs` pill container + `.ctab` segments — the route-backed Articles\|Séries toggle atop both list pages; active segment = `&[aria-current='page']` (filled `--text` on `--bg`) | no |
