---
name: angular-rules
description: Project's in-house Angular conventions (standalone, zoneless, signals, native control flow, separate templates/styles, bilingual i18n, OnPush). TRIGGER when creating/editing an Angular component or service, adding an input/output, handling reactive state, or writing a template/route. SKIP for non-Angular questions.
---
...
signal primitives, OnPush, native control flow) are owned by [`code.md`](../../conventions/code.md) §1–§5.
    ...
    export class CommentItemComponent {
      ...
      public readonly reply = output<string>();             // parent listens → public
...
Inject `I18nService`, read text via `i18n.content()`; a new string is added to **both** locale JSONs.
The typed bridge and `Content` FR/EN contract are owned by
...
Language as a URL prefix via two static `/fr` `/en` trees (never a `:lang` param), the `langResolver`,
and route-as-language-source are owned by
[`architecture.md`](../../conventions/architecture.md). Apply them like this:

- `app.routes.ts`: each tree carries `resolve: { lang: langResolver }` over shared lazy `langChildren()`;
  `/` and `**` redirect to `/fr` with static strings. Features with internal routing expose `*.routes.ts`
...
- the FR/EN toggle **navigates to the twin URL** (swap segment 0), it does **not** call `setLang`
  directly (`NavComponent.switchLang`).
