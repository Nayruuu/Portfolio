...
.PHONY: help install dev build build-prod build-ssg og gen-icons gen-article-bodies gen-read-times i18n optimize-game-art test test-cov e2e lint lint-fix format format-check check-docs clean
...
i18n: ## AI-translate content + article bodies via claude -p, e.g. make i18n LANGS="en"
	cd $(APP) && npm run i18n -- $(LANGS)
