...
- All user-facing **copy** lives in the bilingual content store; this doc describes **structure +
...
A bilingual (FR/EN) **"YouTube-channel" portfolio** for a full-stack **.NET / Angular / Azure** dev
...
channel header, the tabs bar, then the routed page inside a `.main` wrapper, and the functional
`.konami` keyboard-shortcut hint at the bottom (home route only). The theme is applied on load (writing `data-theme` onto the document)
...
theme: `background:rgb(250 249 246 / 88%)`.

Members left→right (visual notes):

- **Menu** icon button (`.nav__icon-btn`, 36px circle, transparent → hover `--surface`).
...
- **Actions** (`.nav__actions`, gap 8px): **theme toggle** (moon icon in light / sun in dark, title +
  aria from content); **FR/EN pill** (`.nav__lang`, two buttons with `aria-pressed` = current lang;
  the active one gets accent fill `color:#0a0a0b`, and the `FR`/`EN` labels are fixed literals) — it
  **navigates** to the twin URL by replacing only the first URL segment (the destination tree then
  syncs the language before render), it does **not** switch the language directly; **bell** icon
  button; **`S` avatar** (32px circle, the brand-red sphere
...
**Below `md`** the search pill and the whole actions cluster (theme · FR/EN · bell · avatar) collapse
behind the existing **☰** button into a **slide-in drawer** over a dark **scrim**; the ☰ exposes its
disclosure ARIA (`aria-expanded` / `aria-controls`) **only** at these compact widths (where it truly
toggles the drawer), and the drawer closes on **Escape**, a **scrim tap**, or a **language tap**. At
`md` and up the drawer dissolves (its container becomes `display:contents`) and the search + actions sit
inline exactly as listed above.
...
Language is a **URL prefix via two explicit static trees** (`/fr`, `/en`) — **never** a `:lang`
param (a parameter-first parent route breaks Angular's native prerenderer → empty `<router-outlet>`).

- **Routes**: two top-level trees, `/fr` and `/en`, each reading its own leading path segment to sync
  the language before the routed component renders (the **only** place routing changes the language),
  and sharing the **same lazy children**, built fresh per tree, in order: Home `''` (lazy component),
  `articles` and `series` (lazy sub-trees — each a list + detail), then `about`, `stack`, `contact`
  (lazy components). Root `''` and any unknown path redirect to `/fr` with **static** strings
  (build-evaluable for the static prerender).
...
- **App config**: zoneless change detection, the router (with component-input-binding), and client
  ...
  routes.
- **Prerender manifest**: the four detail routes (`{fr,en}/{articles,series}/:slug`) are prerendered
  with their slugs enumerated from the **FR** content; a final catch-all prerenders the explicit
  static pages (`/fr`, `/en`, `/fr/about`, …).
...
- **Controls row** (`.player__row`, gap 8px → **12px at `sm`**, `color:#fff`): play/pause (22px) ·
  next-chapter (20px, wraps) · **decorative volume** `.player__vol` (bar expands `0→60px` on hover,
  fill 70%) · `mm:ss / mm:ss` time + `· <b>{current chapter title}</b>` (accent-hot,
  `__chapter-now`) · spacer · **3 inert icon buttons** captions/gear/pip (20px) + the **working fullscreen toggle** (icon `full` ↔ `full-exit`, label `aria.fullscreen` ↔ `aria.exitFullscreen`). **Phone trim**
  (all restored at `sm`): the volume, the chapter-now label, and the three aux buttons
  (captions/gear/pip, `__btn--aux`) are hidden so play/next/time/fullscreen fit unclipped. All
  icon-button accessibility labels come from content.
...
- **video-meta** (`sd-video-meta`): the featured title as an `h2`; author row (`S` avatar
  ...
  + Subscribe (`btn btn--primary btn--sm`) + actions (`<sd-like-bar>`, **Share**, **Download-CV**,
  **Save**, **more** — all `btn btn--sm`). **Mobile-only bio** (`.video-meta__bio`, between the
...
- **comments** (`sd-comments`/`sd-comment`): a header **toggle button** (`.comments__head` — comments
  ...
  button) + an **unwired** input row (`S` avatar + placeholder) + a list of `<sd-comment>`; the input
  + list render only while expanded. **Collapsed by default on phones** (the initial state is
  `!ViewportService.isCompact()` — expanded on desktop, where the section reads exactly as before;
  the chevron is hidden at `md`). Seed = **4 comments, 1 pinned**.
  ...
  pill, an optional `📌 {pinned label}`, a `__when` mono 11px timestamp, body 13.5px, actions (a like
  toggle showing the base likes plus the user's vote, dislike, reply — mono 11px).
- **up-next** aside (`sd-up-next`): header (up-next title · `<b>{read-next}</b>` + a **decorative
  inert "ON" toggle** with accent track) + the **first 5** articles as `.vid-card` rows
...
ASCII, identical FR/EN, = the Markdown filename stem), bound as a required string input; the detail
...
- **3px sticky reading-progress bar** `.article-detail__progress` at **`top:56px`** (under the nav),
  accent, width = the scroll position over the scrollable height, capped at 100% (browser-only).
...
- **Topbar** `.article-detail__topbar` (flex **wrap**, gap 8px — the actions wrap under the back
  ...
  list) + `__actions` (Save + Share, labels from content).
...
- **Bodies are real Markdown**: stored as raw `.md` files (one per slug, FR + EN), imported as text
  and indexed by slug. An in-house Markdown subset is parsed into typed blocks — headings,
...
- **`sd-icon`** = a fixed inline-SVG set of **28** names: play, pause, next, vol, captions, gear, pip,
  full, full-exit, thumbs-up, thumbs-down, share, download, save, search, bell, mail, more, menu, sun, moon,
  chevron-up, chevron-down, home, articles, series, about, layers — `viewBox 0 0 24 24`, a size input (default **18**), 2px `currentColor`
...
- **No convention debt remains.** Article filler was replaced by **real per-article Markdown**; the
  ...
  2026-06-04 sweep. Remaining dynamic style bindings are data-driven + the deliberate reveal fade-in.
- **Intentional, not debt** (fixed brand art / data — reproduce as-is): the channel-header h1 role
  ...
  row, the `contact@super-dev.app` contact address, the up-next
  `ON` toggle, and the player's `4K · HDR` / inert captions·gear·pip buttons.
...
- **SEO / site**: a fixed site origin/name, the default OG image, the author identity, per-language OG
  locales, and the FR↔EN twin mapping, plus helpers to absolutize a path and to swap a URL's leading
  language segment for its twin.
...
- **Content gateway**: the single API seam, **mocked** today — it exposes a synchronous peek that
  returns the bundled FR/EN content and an async fetch that resolves the same after a ~300ms delay.
  This is the one place to change for a real .NET API. It reads its base URL from the build
  environment.
- **Content store**: a signal store holding the current language + content + a loading flag, seeded
  from the stored language preference (localStorage-only; EN only if explicitly stored, else FR — so
  prerender and tests default deterministically to FR) and the synchronous peek, then revalidated
...
- **Viewport**: a single reactive "below `md`?" flag from a `matchMedia` listener — `false` on the
  server (SSR/prerender-safe), live in the browser. Drives the nav drawer's disclosure ARIA and the
  comments' collapsed-by-default start on phones; pure-CSS responsive behavior never goes through it.
- **SEO**: sets the title, Open Graph / Twitter / canonical / hreflang tags per route plus optional
  ...
  emits three alternates: the current language at the path, the twin language at the twin path, and
  `x-default` at the FR path. Structured-data is injected into a single script element via text
