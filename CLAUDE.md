...
A multilingual (FR/EN — extensible via the `LANG` value set) **"YouTube-channel" portfolio** for
...
| `make optimize-game-art` | convert served game PNGs → WebP (shrinks the in-player game's image payload) |
| `make gen-icons` | regenerate the typed icon set (`icon-set.ts`) from `icons/*.svg` |
| `make i18n LANGS="en"` | AI-translate `content.fr.json` + article bodies → `content.<lang>.json` / `<slug>.<lang>.md` via `claude -p` (committed) |
...
- **Layered, screaming architecture** under `client/src/app/`, **imports point inward only**
  ...
  - `shared/` — cross-feature UI components (`icon`, `code-block`, `inline-runs` — presentational; `like-bar` — the interactive vote bar; `reviews` — the Malt recommendations block, on home + about; `command-palette` — the desktop-only ⌘K command palette; `content-tabs` — the route-backed Articles|Séries toggle atop the two list pages) **only**.
  ...
  - `features/` — one **lazy-loaded** folder per feature (`home`, `articles`, `series`, `projects`,
    `about`, `stack`, `contact`). Tab bar = **Accueil · Articles · Réalisations · À propos · Stack ·
    Contact**: `projects` is the **Réalisations** tab, and `series` has **no tab of its own** — it shares
    the *Articles* tab via the route-backed `content-tabs` toggle (both `/articles` and `/series` stay
    real, prerendered routes; the Articles tab reads active on `/series` too). The `/about` "Projets" card
    is kept as a secondary entry — plus the hidden `bsp-demo` game, now just the thin **mount component**
...
- **Routing & i18n**: language is a URL prefix (`/fr`, `/en`, …), route-driven; one static
