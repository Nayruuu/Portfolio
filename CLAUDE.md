...
A bilingual (FR/EN) **"YouTube-channel" portfolio** for a full-stack **.NET / Angular / Azure**
developer — brand `super-dev` / `.app`, selector prefix **`sd-`**. First and foremost a **technical
showcase**: the code must stay exemplary against the latest Angular 21 patterns.
...
| [`.claude/conventions/testing.md`](.claude/conventions/testing.md) | The test contract: Vitest patterns + exact coverage thresholds, the `core/` 100 % guard, Playwright config (chromium + **mobile** + **webkit** projects) + the 15 specs + 16 visual baselines (8 desktop + 8 mobile) + re-baseline discipline, the prerender guard. |
...
- **Layered, screaming architecture** under `client/src/app/`, **imports point inward only**
  ...
  - `domain/` — types/value-sets + the bilingual `Content` contract; **depends on nothing**.
  - `core/` — UI-less client/infra logic: `api/` (the .NET-API seam), `services/` (signal/SignalStore
    state), `lib/` (pure functions, 100 % tested), `content/` (the FR/EN JSON + typed bridge + `.md`
    article bodies).
  - `shared/` — cross-feature presentational components (`icon`, `code-block`, `inline-runs`).
  - `layout/` — the shell (`nav`, `channel-header`, `tabs-bar`).
...
- **Routing & i18n**: language is a URL prefix (`/fr`, `/en`), route-driven; UI text comes from the typed
  FR/EN `Content` bridge. Rules → `architecture.md` §7 (routing/SEO placement + route-as-language) +
  `code.md` §5 (the no-`lang()`-ternary text rule); route/SEO **content** → `PRODUCT.md`.
