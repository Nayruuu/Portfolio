...
test('an English deep link renders English UI, and the picker offers every language', async ({
  page,
}) => {
  await page.goto('/en');
  await expect(page).toHaveURL(/\/en$/);
  await expect(page.getByRole('tab', { name: 'Home' })).toBeVisible();
  // The dropdown toggle shows the current language; opening it lists every language.
  await expect(page.locator('.nav .prefs__lang-toggle')).toHaveText(/EN/);
