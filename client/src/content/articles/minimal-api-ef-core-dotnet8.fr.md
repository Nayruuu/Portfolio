...
s'appelle **`MapGroup`**. Chaque ressource a son groupe, avec son préfixe, ses
...
l'applique au démarrage avec `db.Database.MigrateAsync()`, jamais avec `EnsureCreated`, qui
...
opaque, on retourne une **union de résultats typés** : la signature documente les codes HTTP
...
Pour les tests d'intégration de bout en bout, `WebApplicationFactory<T>` monte
l'application complète en mémoire et permet de taper les vrais endpoints.

> Une Minimal API n'est pas une API au rabais. Bien découpée en groupes et en résultats
> typés, elle expose **moins de cérémonie pour plus de garanties**, et c'est ce qu'on
> attend d'un framework moderne.
