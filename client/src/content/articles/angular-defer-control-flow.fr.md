Angular a remplacé `*ngIf` et `*ngFor` par un nouveau control flow, livré avec `@defer`.
La combinaison change ce qui finit dans le bundle initial : on n'expédie au démarrage que
le JavaScript réellement nécessaire au premier rendu, le reste arrive à la demande.
...
Une page lourde (éditeur de code, graphiques, carte) peut ainsi sortir 100 à 200 ko du
bundle initial, qui ne se téléchargent que si l'utilisateur scrolle jusque-là. Le gain se
mesure directement sur le **Largest Contentful Paint** et le temps d'interactivité.

La doc détaille chaque déclencheur dans le
...
> Le control flow rend l'intention lisible, et `@defer` attache un coût explicite à chaque
> morceau de template. Plutôt que de tout charger « au cas où », on déclare quand chaque bloc
> mérite son JavaScript, et le démarrage s'allège.
