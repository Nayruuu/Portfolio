...
describe('tokenize', () => {
  ...
  it('classes a known sql keyword (SELECT) as keyword token (kind="k")', () => {
    const tokens = tokenize('SELECT c.Id FROM dbo.Customers', 'sql');
    const keyword = tokenOf(tokens, 'SELECT');

    expect(keyword).toBeDefined();
    expect(keyword?.kind).toBe('k');
  });

  it('classes a sql -- comment as a comment token (kind="c")', () => {
    // SQL line comments start with `--` (lang-scoped, so a C#/TS `i--` is never a comment).
    const tokens = tokenize('END; -- forward-only', 'sql');
    const comment = tokenOf(tokens, '-- forward-only');

    expect(comment).toBeDefined();
    expect(comment?.kind).toBe('c');
  });

  it('does NOT treat -- as a comment outside sql (csharp decrement stays code)', () => {
    const tokens = tokenize('i--;', 'csharp');
    const comment = tokens.find((token) => token.kind === 'c');
...
describe('LANG_LABEL', () => {
  it('maps known language ids to display labels', () => {
    ...
    expect(LANG_LABEL['sql']).toBe('SQL');
  });
});
