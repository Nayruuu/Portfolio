...
export interface AssetDecoders {
  ...
  /** Hand-sculpted `.vox` collectibles (weapons, vitals, keycards), keyed by their PICKUP_* name to override the 2D icon. */
  loadPickupVox(): Promise<Map<string, Texture>>;
...
export class AssetLoader {
  ...
  public async loadCritical(): Promise<void> {
    ...
    const [env, props, pickups, weaponVox] = await Promise.all([
      ...
      // Collectibles as voxel volumes (weapons, vitals, keycards) — override the 2D icon under the same name (below).
      this.decoders.loadPickupVox(),
    ]);
