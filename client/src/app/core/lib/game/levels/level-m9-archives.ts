...
function buildMap(): { map: MapSource; closetDoorSector: number } {
  ...
  const stacks = { ceilZ: 2.8, light: 90, wallTex: 'DAMAGED' };
  ...
  const pool = { floorTex: 'SLAB', ceilTex: 'NEON', light: 228, wallTex: 'DAMAGED' };
  ...
  const strip = { floorZ: 0.05, ceilZ: 2.8, floorTex: 'STEP', ceilTex: 'CEIL_DAMAGED', light: 113 };
  ...
  const ANTE = b.room(poly([26, 64, 36, 64, 36, 72, 28, 72]), {
    ...
    floorTex: 'SLAB',
    ceilTex: 'CEIL_DAMAGED',
    light: 88, // anchor 2: the vault shaft's 200 leaks through the opening below
    wallTex: 'DAMAGED',
  });
