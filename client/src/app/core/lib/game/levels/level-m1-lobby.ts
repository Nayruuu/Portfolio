...
function buildMap(): { map: MapSource; doorSector: number } {
  ...
  const CONC = b.room(
    ...
    { ...LUX, ceilZ: 4.4, light: 178, wallTex: 'LOBBY', walls: { 1: 'WOOD' } },
  );

  const cornice = { ...LUX, ceilZ: 5.6, light: 253, wallTex: 'WOOD' };
  ...
  const run = { depth: 6, count: 5, zBase: 0, dz: 0.4, ceilZ: 5.6, light: 113, wallTex: 'LOBBY' };
  ...
  const LOUNGE = b.room(
    ...
    { ...LUX, ceilZ: 3.6, light: 158, wallTex: 'LOBBY', walls: { 6: 'WOOD' } },
  );
  ...
  const wood = { ceilZ: 3.6, ceilTex: 'CEIL_LUX', light: 158, floorTex: 'STEP', wallTex: 'WOOD' };
  const carpet = { ...LUX, ceilZ: 3.6, light: 158, floorTex: 'CARPET', wallTex: 'METAL' };
  ...
  const slab = { ...UPPER, ceilZ: 5.0, light: 163, wallTex: 'GLASS_INT' };
  const DOOR = b.room(rect(26, 68, 34, 70), slab);

  b.connect(DOOR, STEP[4]);
  const hall = { ...UPPER, ceilZ: 7, light: 203, wallTex: 'LOBBY' };
