...
/** `floorZ`/`ceilZ` change at runtime (animated doors) and `light` (dying-tube flicker). */
type SectorHeights = readonly {
...
/** A held zone with its AUTHORED sector state, restored when the zone is demoted from primary to neighbor
 *  (live door/flicker mutations must not linger in a neighbor render). */
interface HeldZone {
...
/** Build (and hold) a zone, recording the authored sector state for later restore. */
function hold(key: string, source: MapSource): void {
  zones.set(key, {
    map: buildBsp(source),
    base: source.sectors.map((s) => ({ floorZ: s.floorZ, ceilZ: s.ceilZ, light: s.light })),
  });
}

/** Undo the primary-time door/flicker mutations — reset a held zone's sectors to the authored state. */
function restoreHeights(zone: HeldZone): void {
  const secs = zone.map.source.sectors as unknown as {
    floorZ: number;
    ceilZ: number;
    light: number;
  }[];
  ...
  for (let i = 0; i < zone.base.length; i++) {
    ...
    secs[i].light = zone.base[i].light;
  }
}
...
addEventListener('message', ({ data }: MessageEvent<Inbound>) => {
  if (data.type === 'init') {
    ...
    // Sync the live sector state (doors mutate `ceilZ`, flicker mutates `light`); the renderer reads these
    // straight off — without this the worker/GPU would render a static light while the CPU flickers, diverging.
    const secs = zone.map.source.sectors as unknown as {
      floorZ: number;
      ceilZ: number;
      light: number;
    ...
    for (let i = 0; i < data.sectors.length && i < secs.length; i++) {
      ...
      secs[i].light = data.sectors[i].light;
    }
