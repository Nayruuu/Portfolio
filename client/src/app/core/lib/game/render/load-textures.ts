...
function decoderPool(): Worker[] | null {
  ...
  atlasPool = Array.from({ length: ATLAS_WORKERS }, () => {
    ...
    // null → every later decode takes its main-thread path) and re-route EVERY in-flight decode through
    // its fallback — terminating the siblings below means their own onerror won't fire to drain theirs.
    worker.onerror = (): void => {
      atlasPool?.forEach((sibling) => sibling.terminate()); // else retiring the pool to null orphans the healthy siblings
      atlasPool = null;
...
/** Called from RenderHost.dispose so a finished game session leaves no idle decode workers behind (the
 *  render worker pool is torn down the same way); any in-flight decode settles to null, not a hung promise. */
export function disposeDecoderPool(): void {
  atlasPool?.forEach((worker) => worker.terminate());
  atlasPool = undefined; // undefined, not null: null means "retired for good", undefined lets it rebuild
...
/** COLLECTIBLES as voxel volumes — weapons, ammo boxes, vitals, keycards: a hand-sculpted `.vox` next
 *  to (or in place of) the 2D art overrides the spinning icon under the same PICKUP_* texture name
 *  (the renderer volume-renders any voxel grid). Absent / 404 → the icon billboard stays, so each
 *  pickup joins the doctrine the day its sculpt lands. The grid is used AS-IS — NO AO bake, NO
 *  regrade: the model carries its own colour/shading; the engine only frames (trim) and scales
 *  (budget + the spec's display height × the model's own ratio), never repaints. */
...
export async function loadPickupVox(): Promise<Map<string, Texture>> {
  const out = new Map<string, Texture>();
  ...
  // Vitals + keycards: the vox sits NEXT TO the 2D art (same `_rot` basename, `.vox` extension).
  // The mental figurine has no sculpt yet — its 404 keeps the billboard, like any absent model.
  for (const spec of [
    HEALTH_LARGE_SPEC,
    HEALTH_SMALL_SPEC,
    MENTAL_LARGE_SPEC,
    ...
    await intake(spec.url.replace(/\.webp$/, '.vox'), spec.texName, spec.worldHeight);
  }
