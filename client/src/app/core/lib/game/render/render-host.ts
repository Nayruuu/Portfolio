...
import { disposeDecoderPool, proceduralTextures } from './load-textures';
...
export interface RenderHostBootstrap {
  ...
  readonly pinWorkers: number | null; // `?workers=N` — pin the CPU pool's active count (thread-scaling bench)
  readonly camera: Camera;
...
export class RenderHost {
  ...
  public bootstrap(setup: RenderHostBootstrap): void {
    ...
    this.pool = pool;
    // `?workers=N` pins the active count (the thread-scaling bench control); `?nogov=1` pins full workers.
    // Either one disables the governor so it can't move the count back under measurement.
    if (pool !== null && setup.pinWorkers !== null) {
      pool.setWorkers(setup.pinWorkers);
    }
    this.governor =
      pool === null || setup.noGovernor || setup.pinWorkers !== null
        ? null
        : initialRenderGovernor(pool.threads);
