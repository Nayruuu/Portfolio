...
export interface VitalSpec {
  ...
  readonly worldHeight: number; // the 2D billboard height (pre-decode fallback)
  readonly voxHeight?: number; // the VOLUME's display height when a vox is loaded — decoupled from the
  // billboard so a wide/flat model (the medkit) isn't blown up by its own lateral ratio; defaults to worldHeight
...
export const HEALTH_LARGE_SPEC: VitalSpec = {
  ...
  worldHeight: 0.6,
  voxHeight: 0.3, // the medkit box is wide+flat (lat/height 2.17): 0.6 → 1.3-wide crate; 0.3 reads as a medkit
  aspect: 359 / 269,
  amount: VITAL_LARGE,
  spin: true,
};
...
// (the vox path renders a rotating VOLUME instead and ignores frames; see loadPickupVox).
