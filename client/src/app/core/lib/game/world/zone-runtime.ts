...
import type { Door, Flicker, SeamEdge, SlidingDoor, WarmZone, ZoneExit } from './zone-world';
...
export class ZoneRuntime {
  ...
  // Same live-sectors contract as applyDoors: re-stamp each buzzing sector's light so the renderer reads the
  // modulated value off the live sectors this frame. The sector index seeds the noise, desyncing the tubes.
  public applyFlicker(
    flickers: readonly Flicker[],
    sectors: MutableSector[],
    timeMs: number,
  ): void {
    for (const flicker of flickers) {
      sectors[flicker.sector].light = flickerLight(flicker.baseLight, timeMs, flicker.sector);
    }
  }
  ...
  private buildWorld(zone: ZoneLoad, snap: ZoneSnapshot | null): WarmZone {
    ...
    return {
      ...
      enemyShots: [],
      doors,
      flickers: this.buildFlickers(zone.level),
  ...
  // Capture the AUTHORED light once (never a live/animated value) — the buzz always modulates the pristine base.
  private buildFlickers(level: Level): Flicker[] {
    return (level.flicker ?? []).map((sector) => ({
      sector,
      baseLight: level.map.sectors[sector].light,
    }));
  }
