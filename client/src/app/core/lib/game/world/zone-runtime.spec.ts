...
import { flickerLight } from '../flicker';
...
describe('ZoneRuntime — dying-tube light flicker', () => {
  const KEY = 'synthflicker';

  afterEach(() => {
    delete (LEVELS as Record<string, Level>)[KEY];
  });

  it('captures each buzzing sector’s AUTHORED base light and modulates it live per frame', () => {
    (LEVELS as Record<string, Level>)[KEY] = { ...SYNTHETIC_LEVEL, flicker: [0] };
    const { runtime } = makeRuntime();

    runtime.loadZone(KEY);

    // The base is captured off the pristine authored sector (light 200), seeded by the sector index.
    expect(runtime.world.flickers).toEqual([{ sector: 0, baseLight: 200 }]);

    runtime.applyFlicker(runtime.world.flickers, runtime.world.sectors, 500);
    expect(runtime.world.sectors[0].light).toBe(flickerLight(200, 500, 0));

    // A second frame modulates the AUTHORED base again — the earlier stamp never becomes the new base.
    runtime.applyFlicker(runtime.world.flickers, runtime.world.sectors, 900);
    expect(runtime.world.sectors[0].light).toBe(flickerLight(200, 900, 0));
    expect(runtime.world.sectors[0].light).toBeGreaterThanOrEqual(0);
    expect(runtime.world.sectors[0].light).toBeLessThanOrEqual(255);
  });

  it('warms no flicker on a floor that authored none (the empty default)', () => {
    const { runtime } = makeRuntime();

    runtime.loadZone('m1');

    expect(runtime.world.flickers).toEqual([]);
  });
});
