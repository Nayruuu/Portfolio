...
import type { Door, Flicker, SlidingDoor, WarmZone, ZoneExit } from './zone-world';
...
interface TestZone {
  ...
  applyFlicker: ReturnType<
    typeof vi.fn<(flickers: readonly Flicker[], sectors: MutableSector[], timeMs: number) => void>
  >;
}
...
function setup(worldOverrides: Partial<WarmZone> = {}): Harness {
  ...
  const zone: TestZone = {
    ...
    applyFlicker:
      vi.fn<(flickers: readonly Flicker[], sectors: MutableSector[], timeMs: number) => void>(),
  };
...
describe('PickupRuntime — doors', () => {
  ...
  it('re-stamps the flickering sectors each frame off a monotonic buzz clock (ms)', () => {
    const { pr, world, zone } = setup({ flickers: [{ sector: 0, baseLight: 200 }] });

    pr.stepDoors(0.1);
    pr.stepDoors(0.25);

    expect(zone.applyFlicker).toHaveBeenNthCalledWith(1, world.flickers, world.sectors, 100);
    expect(zone.applyFlicker).toHaveBeenNthCalledWith(2, world.flickers, world.sectors, 350);
  });
});
