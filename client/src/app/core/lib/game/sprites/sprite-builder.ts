...
import { ENEMY_RECOIL, HIT_FLASH_DURATION, PICKUP_VOX_SPIN } from '../game-tuning';
...
export function buildWorldSprites(input: WorldSpritesInput): Sprite[] {
  ...
  for (const v of world.vitals) {
    const voxAspect = input.voxAspects?.get(v.spec.texName);
    ...
    sprites.push(
      voxPickupSprite(
        ...
        v.z,
        v.spec.texName,
        v.spec.voxHeight ?? v.spec.worldHeight,
        voxAspect,
        v.age,
      ),
    );
  ...
  for (const k of world.keycards) {
    const voxAspect = input.voxAspects?.get(k.spec.texName);
    ...
    sprites.push(
      voxPickupSprite(k.x, k.y, k.z, k.spec.texName, k.spec.worldHeight, voxAspect, k.age),
    );
  }
