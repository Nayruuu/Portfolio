...
describe('vitals + keycards — the vox branch', () => {
  it('renders a vital as a rotating volume when its vox is loaded, billboard otherwise', () => {
    const vital = {
      x: 3,
      y: 4,
      z: 0,
    ...
    const build = (voxAspects?: ReadonlyMap<string, number>) =>
      buildWorldSprites({
        world: { ...fixtureWorld(), vitals: [vital] },
        viewX: 0,
        viewY: 0,
        voxAspects,
      }).find((s) => s.tex === 'PICKUP_HEALTH_SMALL');

    const flat = build();
    const voxed = build(new Map([['PICKUP_HEALTH_SMALL', 0.8]]));

    expect(flat?.voxel).toBeUndefined(); // no vox → the spinning billboard
    expect(flat?.cols).toBe(6);
    expect(voxed?.voxel).toBe(true); // vox → a true volume…
    expect(voxed?.width).toBeCloseTo(0.45 * 0.8, 5); // …sized by the model's own ratio
    expect(voxed?.height).toBe(0.45); // no voxHeight → the spec's display height
    expect(voxed?.facing).toBeGreaterThan(0); // …turning with its age
  });

  it('sizes a vital vox by its OWN voxHeight when set, leaving the 2D billboard at worldHeight', () => {
    // A wide/flat model (the medkit, lat/height 2.17) sized by its billboard height reads as a crate;
    // voxHeight decouples the volume's display size from the 2D icon height, like the weapons' knob.
    const vital = {
      x: 3,
      y: 4,
      z: 0,
    ...
    const build = (voxAspects?: ReadonlyMap<string, number>) =>
      buildWorldSprites({
        ...
        viewY: 0,
        voxAspects,
      }).find((s) => s.tex === 'PICKUP_HEALTH_LARGE');

    expect(build()?.height).toBe(0.6); // billboard fallback keeps worldHeight
    const voxed = build(new Map([['PICKUP_HEALTH_LARGE', 2.17]]));

    expect(voxed?.height).toBe(0.3); // the volume takes voxHeight, not worldHeight
    expect(voxed?.width).toBeCloseTo(0.3 * 2.17, 5);
  });

  it('renders a keycard as a rotating volume when its vox is loaded, billboard otherwise', () => {
    const card = {
      x: 1,
      y: 2,
      z: 0,
    ...
    const build = (voxAspects?: ReadonlyMap<string, number>) =>
      buildWorldSprites({
        world: { ...fixtureWorld(), keycards: [card] },
        viewX: 0,
        viewY: 0,
        voxAspects,
      }).find((s) => s.tex === 'PICKUP_KEYCARD_EMPLOYEE');

    const flat = build();
    const voxed = build(new Map([['PICKUP_KEYCARD_EMPLOYEE', 0.55]]));

    expect(flat?.voxel).toBeUndefined();
    expect(flat?.cols).toBe(6);
    expect(voxed?.voxel).toBe(true);
    expect(voxed?.width).toBeCloseTo(0.65 * 0.55, 5);
    expect(voxed?.facing).toBeGreaterThan(0);
  });
});
