...
const KEYWORDS: Record<CodeLang, RegExp> = {
  ...
  sql: /\b(SELECT|FROM|WHERE|INSERT|INTO|VALUES|UPDATE|DELETE|SET|CREATE|OR|ALTER|DROP|PROCEDURE|FUNCTION|VIEW|TABLE|INDEX|AS|ON|AND|NOT|NULL|IS|IN|EXISTS|BEGIN|END|DECLARE|RETURN|RETURNS|IF|ELSE|JOIN|INNER|LEFT|RIGHT|OUTER|GROUP|ORDER|BY|HAVING|DISTINCT|PRIMARY|KEY|FOREIGN|REFERENCES|DEFAULT)\b/,
};
...
export function tokenize(line: string, lang: CodeLang): Token[] {
  ...
  // SQL comments start with `--`; every other language uses `//` or `#`. Kept lang-scoped
  // so a C#/TS decrement (`i--`) is never mistaken for a comment.
