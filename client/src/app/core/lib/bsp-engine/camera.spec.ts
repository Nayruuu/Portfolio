...
describe('camera projection', () => {
  ...
  it('maps world points into camera space (forward along view, side to the left)', () => {
    expect(toCamera(ORIGIN, { x: 5, y: 0 })).toEqual({ forward: 5, side: 0 });
    expect(toCamera(ORIGIN, { x: 0, y: 5 })).toMatchObject({ forward: 0, side: 5 });
  ...
  it('projects a dead-ahead point to screen centre, a left point further left', () => {
    ...
    expect(projectColumn({ forward: 5, side: 0 }, 320, focal)).toBeCloseTo(160, 6);
    expect(projectColumn({ forward: 5, side: 5 }, 320, focal)).toBeCloseTo(0, 6);
  });

  it('projects heights above the eye above the horizon and below below it', () => {
    const focal = focalFor(320, Math.PI / 2);
    const ceil = projectRow(1, 5, ORIGIN, 200, focal);
    const floor = projectRow(0, 5, ORIGIN, 200, focal);

    expect(ceil).toBeLessThan(100);
