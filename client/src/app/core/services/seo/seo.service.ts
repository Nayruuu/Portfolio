...
export class SeoService {
    ...
    this.setJsonLd([
      {
        ...
        image: [image],
        author: this.personFor(data.lang),
        publisher: {
          '@type': 'Organization',
          name: SITE_NAME,
          url: SITE_ORIGIN,
  ...
  public setSiteJsonLd(lang: Lang): void {
    this.setJsonLd([this.siteNode(lang), this.personFor(lang)]);
  }
  ...
  /** The canonical Person with its CV `subjectOf` pointed at the current locale's PDF (fr/en). */
  private personFor(lang: Lang): typeof PERSON {
    return {
      ...PERSON,
      subjectOf: { ...PERSON.subjectOf, url: `${SITE_ORIGIN}/cv/stephane-de-todaro-${lang}.pdf` },
    };
  }
