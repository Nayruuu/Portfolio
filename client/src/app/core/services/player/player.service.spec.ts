...
describe('PlayerService', () => {
  ...
  it('setRate scales the tick increment (2× advances 0.2 per 100 ms)', () => {
    vi.useFakeTimers();
    const svc = TestBed.inject(PlayerService);
    const appRef = TestBed.inject(ApplicationRef);
    ...
    svc.seek(0);
    svc.setRate(2);
    expect(svc.rate()).toBe(2);

    vi.advanceTimersByTime(100);
    expect(svc.time()).toBeCloseTo(0.2, 5);
  });

  it('toggleMini flips the mini-player flag and closeMini forces it off', () => {
    const svc = TestBed.inject(PlayerService);

    expect(svc.mini()).toBe(false);
    svc.toggleMini();
    expect(svc.mini()).toBe(true);
    svc.toggleMini();
    expect(svc.mini()).toBe(false);
    svc.toggleMini();
    svc.closeMini();
