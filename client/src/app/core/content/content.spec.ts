...
const LOCALES = { fr: FR, en: EN } satisfies Record<Lang, Content>;
...
describe('Content — cross-locale alignment', () => {
  ...
  for (const lang of NON_SOURCE) {
    ...
    it(`${lang}.about.realisations has the same item count as FR`, () => {
      expect(LOCALES[lang].about.realisations.items.length).toBe(
        FR.about.realisations.items.length,
      );
    });
