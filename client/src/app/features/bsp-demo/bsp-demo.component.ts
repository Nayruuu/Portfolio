...
// native status-bar art dims → the backing store keeps the bar's 5.24:1 aspect (2117 is also the largest
// art tier, so it doubles as the cap on the dpr-scaled backing store — no detail exists beyond it)
...
export class BspDemoComponent {
  ...
  protected readonly fireButton = viewChild.required<ElementRef<HTMLButtonElement>>('fire');
  protected readonly weaponButton = viewChild.required<ElementRef<HTMLButtonElement>>('weaponBtn');
  // The floating twin-stick joystick view (touch only) — null while no move-thumb is down.
  protected readonly joystick = signal<JoystickView | null>(null);
  ...
  // `?workers=N` pins the CPU pool's active thread count (thread-scaling benchmark; parsed here, off the
  // LevelParams contract). Null = let the pool/governor decide.
  private readonly pinWorkers: number | null = (() => {
    const raw = Number(
      new URLSearchParams(typeof location === 'undefined' ? '' : location.search).get('workers'),
    );
  ...
  private buildInputHooks(): InputControllerHooks {
    return {
      ...
      onJoystick: (view) => this.joystick.set(view),
    };
  }
  ...
  private bindInputListeners(canvasEl: HTMLCanvasElement): void {
    ...
    // Twin-stick touch (mobile): the canvas halves are the move/look zones — listeners on the CANVAS, not
    // window, so the fixed fire/weapon buttons keep their own touches. `passive:false` lets preventDefault
    // block scroll/zoom.
    canvasEl.addEventListener('touchstart', input.onTouchStart, { passive: false });
    canvasEl.addEventListener('touchmove', input.onTouchMove, { passive: false });
    canvasEl.addEventListener('touchend', input.onTouchEnd, { passive: false });
    canvasEl.addEventListener('touchcancel', input.onTouchEnd, { passive: false });
    const fireEl = this.fireButton().nativeElement;
    const weaponEl = this.weaponButton().nativeElement;

    fireEl.addEventListener('touchstart', input.onFireDown, { passive: false });
    fireEl.addEventListener('touchend', input.onFireUp, { passive: false });
    fireEl.addEventListener('touchcancel', input.onFireUp, { passive: false });
    weaponEl.addEventListener('touchstart', input.onWeaponButton, { passive: false });
    this.resizeHud();
  }
  ...
  private teardownGame(canvasEl: HTMLCanvasElement, hudResize: ResizeObserver): void {
    ...
    canvasEl.removeEventListener('touchstart', input.onTouchStart);
    canvasEl.removeEventListener('touchmove', input.onTouchMove);
    canvasEl.removeEventListener('touchend', input.onTouchEnd);
    canvasEl.removeEventListener('touchcancel', input.onTouchEnd);
    const fireEl = this.fireButton().nativeElement;
    const weaponEl = this.weaponButton().nativeElement;

    fireEl.removeEventListener('touchstart', input.onFireDown);
    fireEl.removeEventListener('touchend', input.onFireUp);
    fireEl.removeEventListener('touchcancel', input.onFireUp);
    weaponEl.removeEventListener('touchstart', input.onWeaponButton);
  ...
  private resizeHud(): void {
    ...
    // Back the store with PHYSICAL pixels (× dpr), capped at the native art width — a CSS-sized canvas is
    // bilinear-upscaled on a high-density phone and the bar reads blurry. DoomHud then picks the tier that
    // covers this width and downscales, so it stays crisp.
