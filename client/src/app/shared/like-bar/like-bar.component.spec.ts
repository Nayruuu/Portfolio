...
describe('LikeBarComponent', () => {
  ...
  it('keys the vote by a language-agnostic page (strips the /<lang> prefix)', async () => {
    history.pushState({}, '', '/fr/articles/my-slug');
    fixture = TestBed.createComponent(LikeBarComponent);
    await fixture.whenStable();

    await internals().cast('up');

    expect(feedback.casts.at(-1)?.page).toBe('/articles/my-slug');
    history.pushState({}, '', '/');
  });
